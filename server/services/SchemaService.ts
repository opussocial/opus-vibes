...
export class SchemaService implements ISchemaService {
  ...
  async createType(data: { name: string, description: string, statuses?: string[], color?: string, icon?: string, settings?: any, properties: any[], allowed_parent_types?: number[] }): Promise<number> {
    const { name, description, statuses, color, icon, settings, properties, allowed_parent_types } = data;
    const slug = slugify(name);
    
    const transaction = db.transaction(() => {
      const typeId = db.prepare("INSERT INTO element_types (name, slug, description, statuses, color, icon, settings) VALUES (?, ?, ?, ?, ?, ?, ?)").run(name, slug, description, statuses ? JSON.stringify(statuses) : null, color || "#6366f1", icon || "Package", settings ? JSON.stringify(settings) : null).lastInsertRowid as number;
      ...
      if (allowed_parent_types && Array.isArray(allowed_parent_types)) {
        const allowCircular = featureService.isFeatureEnabled("allow_schema_circular_dependencies");
        
        for (const parentId of allowed_parent_types) {
          if (parentId === typeId && !allowCircular) throw new Error("A type cannot be its own parent.");
        }
  ...
  async updateType(idOrSlug: string, data: { name: string, description: string, statuses?: string[], color?: string, icon?: string, settings?: any, properties: any[], allowed_parent_types?: number[] }): Promise<void> {
    const { name, description, statuses, color, icon, settings, properties, allowed_parent_types } = data;
    ...
    const transaction = db.transaction(() => {
      db.prepare("UPDATE element_types SET name = ?, slug = ?, description = ?, statuses = ?, color = ?, icon = ?, settings = ? WHERE id = ?").run(name, slugify(name), description, statuses ? JSON.stringify(statuses) : null, color || "#6366f1", icon || "Package", settings ? JSON.stringify(settings) : null, type.id);
